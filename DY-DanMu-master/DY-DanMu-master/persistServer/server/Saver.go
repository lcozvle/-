...
func (s *ItemSaverService) Save(item item.Item, result *string) error {
	...
	if err == nil && err1 == nil {
		...
		Log.Debugf("RPC Count ItemSaver: %d", s.Count)
		*result = "ok"
	} else {
		Log.Errorf("Error Saving item %v: err=%v err1=%v", item, err, err1)
	}
	return err
}
...
func Save(client *elastic.Client, conn *sql.DB, item item.Item, index string) (err error, err1 error) {
	...
	if Payload != nil {
		...
		prepare, err1 := conn.Prepare(fmt.Sprintf("insert ignore into %s(`cid`,`level`,`bl`,`bnn`,`brid`,`col`,`cst`,`dms`,`ifs`,`esIndex`,`hc`,`urlev`,`type`,`sahf`,`lk`,`fl`,`el`,`ct`,`txt`,`uid`,`nn`,`rid`) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", config.MysqlDBName+"."+config.MysqlTableName))
		if err1 != nil {
			return err, err1
		}
		defer prepare.Close()
		sqlResult, err1 := prepare.Exec(Payload["cid"], Payload["level"], Payload["bl"], Payload["bnn"], Payload["brid"],
			...
			Payload["urlev"], Payload["type"], Payload["sahf"], Payload["lk"], Payload["fl"], Payload["el"], Payload["ct"], Payload["txt"], Payload["uid"], Payload["nn"], item.Rid)
		if err1 != nil {
			return err, err1
		}
